...
import { ArrowRight, BadgeCheck, CheckCircle, FileCheck, Globe2, Headphones, Search } from "lucide-react";
import { RFQModal } from "@/components/rfq-modal";
import { stats } from "@/lib/site-data";
export { InlineRFQForm } from "@/components/inline-rfq-form";
  ...
  return (
    <section className="bg-white py-10">
      <div className="container-wide rounded-lg border border-slate-200 bg-slate-50 p-4 shadow-sm">
        <form action="/resources" method="get" className="flex items-center gap-3 rounded-md bg-white px-4 py-4">
          <Search className="h-5 w-5 text-electric" />
          <input name="q" className="w-full bg-transparent font-semibold outline-none" placeholder={placeholder} />
          <button type="submit" className="rounded-md bg-navy-900 px-5 py-2 text-sm font-black uppercase text-white">Search</button>
        </form>
      </div>
    </section>
  );
